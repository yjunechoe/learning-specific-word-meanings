window.PennController._AddElementType("SallyMessageBar", function(PennEngine) {
    ...
    this.actions = {
      sallySay: function(resolve, speech = "") {
        this.speechBubble.css("visibility", "visible")
        if (speech !== "") {
          this.textp.text(speech)
        }
        resolve()
      },
      sallyQuiet: function(resolve) {
        this.speechBubble.css("visibility", "hidden")
        resolve()
      }
    };
